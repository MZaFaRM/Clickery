...
def print_title():
    for line in title_card:
        rich_print(Align(line, align="center"))
...
def startup(argv):
    ...
    rich_print()
    text = Align(
        ":Party_Popper: [bold #8D72E1] SUCCESS :Party_Popper: ", align="center")
    rich_print(
        Panel(text, subtitle="[#6D9886]The End", subtitle_align="right"))
    ...
    for action in config.record:
        for key, value in action.items():
            ...
            # For Clicking
                ...
                rich_print(
                    f" [#7D9D9C italic]Inserting {action['click']} click...")
                ...
                recorded.add_row(
                    ":palm_tree:",  f"[#829460 BOLD]{action['click']} CLICKED AT [/#829460 BOLD]{current_position}".upper(
                    ), f"{i}"
                )
            ...
            # For dragging with cursor
                ...
                pyautogui.dragTo(
                    position["x"], position["y"], config.Drag_Speed)
                print("\033[A                               \033[A")
                recorded.add_row(
